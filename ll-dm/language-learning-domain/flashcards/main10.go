// putting time stamp to the session
package main
		...
		if m.AppState.ShowMainMenu {
			...
			if m.AppState.Session.WriteAnswer {
				...
				case key.Matches(msg, keys.Save):
					// Save and quit
					filename := fmt.Sprintf("session_%s.json", time.Now().Format("2006-01-02-15-04"))
					filepath := fmt.Sprintf("./sessions/%s",filename)
					if err := SaveSession(m.AppState.Session, filepath); err != nil {
...
func SaveSession(sessionState SessionState, filename string) error {
